...
export default function AdminTeachers() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (teacherError) {
        setMessage('Erreur lors de la création de l\'enseignant');
        setSaving(false);
        return;
      }
      ...
      if (teacherData && teacherData[0] && formData.classes.length > 0) {
        ...
        if (classError) {
          setMessage('Enseignant créé mais erreur lors de l\'association aux classes');
          setSaving(false);
          return;
        }
      }
